# pylint: disable=broad-exception-caught,wrong-import-position,redefined-outer-name
r"""
   ___  ___  _____              
...
st.cache_data()
def list_bedrock_embedding_models():
    """
    ...
    try:
        ...
        return [
            model['modelId']
                for model in client.list_foundation_models(
...
st.cache_data()
def list_google_embedding_models():
    """
    Returns a list of embedding models available in Google GenAI
    """
    try:
        client = genai.Client()
        return [
...
def list_openai_embedding_models():
    """
    Returns a list of embedding models available in OpenAI
    """
    try:
...
st.cache_data()
def list_google_text_models():
    """
    Returns a list of text models available in Google GenAI
    """
    try:
        client = genai.Client()
        return [
...
st.cache_data()
def list_openai_text_models():
    """
    Returns a list of text models available in OpenAI
    """
    try:
        client = openai.OpenAI()
        return [
...
def embedding_function():
    ...
    if model_provider == "Amazon Bedrock ⛰️":
        ...
        st.session_state.embedding_function = GoogleGenerativeAiEmbeddingFunction(
...
def multiple_queries_expansion(user_query, df_query_original, model):
    """
    Expands a user query by creating multiple sub-queries
    """
    prompt = ChatPromptTemplate.from_messages([
        ("human", "Give me a list of 3 sub-queries related to the query below.\n\n<query>\n{user_query}\n</query>\n\n")
    ])

    if model_provider == "Amazon Bedrock ⛰️":
        model = ChatBedrock(model_id=model)
    elif model_provider == "Google GenAI 🧠":
    ...
    try:
        ...
        st.warning(f"Failed to expand query due to '{exception}', falling back to the naive approach!")
        return [user_query], df_query_original


def generated_answer_expansion(user_query, df_query_original, model):
    """
    ...
    try:
        ...
        st.warning(f"Model failed to expand query due to '{exception}', falling back to the naive approach!")
        return [user_query], df_query_original
...
st.title("RAGMap 🗺️🔍 // #5dgai Edition 🔵🔴🟡🟢")
...
if model_provider == "Amazon Bedrock ⛰️":
    embedding_model = st.selectbox(
        ...
        options=list_bedrock_embedding_models(),
        index=0,
        key="embedding_model",
        on_change=reset,
    ...
    embedding_model = st.selectbox(
        label="Embedding Model",
        options=list_google_embedding_models(),
        index=0,
        key="embedding_model",
        on_change=reset,
        help="The model that turns information (text, image, &c.) \
    ...
    embedding_model = st.selectbox(
        label="Embedding Model",
        options=list_openai_embedding_models(),
        index=0,
        key="embedding_model",
        on_change=reset,
        help="The model that turns information (text, image, &c.) \
...
if st.session_state.projections is not None:
    ...
    if retrieval_strategy != "Naive":
        text_model_provider = st.radio(
            ...
            help="The company or service that provides/hosts the text model",
        )

        if text_model_provider == "Amazon Bedrock ⛰️":
            text_model = st.selectbox(
                label="Text Model",
                options=list_bedrock_text_models(),
                index=0,
                key="text_model",
                on_change=None,
                help="The model that generates the answer",
            )
        elif text_model_provider == "Google GenAI 🧠":
            text_model = st.selectbox(
                label="Text Model",
                options=list_google_text_models(),
                index=0,
                key="text_model",
                on_change=None,
                help="The model that generates the answer",
            ...
            text_model = st.selectbox(
                label="Text Model",
                options=list_openai_text_models(),
                index=0,
                key="text_model",
                on_change=None,
                help="The model that generates the answer",
    ...
    if st.button(label="Search"):
        ...
        if retrieval_strategy == "Naive":
            ...
            chroma_query, df_query = multiple_queries_expansion(query, df_query, text_model)
        elif retrieval_strategy == "Generated Answer":
            chroma_query, df_query = generated_answer_expansion(query, df_query, text_model)
